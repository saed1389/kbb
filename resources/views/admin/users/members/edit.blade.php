...
@section('content')
    ...
    <div class="container-xxl flex-grow-1 container-p-y mb-5">
        <div class="card">
            <h5 class="card-header">{{ $user->first_name.' '.$user->last_name }} Bilgilerini Güncelle</h5>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-8">
                    ...
                    <form class="" action="{{ route('members.update', $user->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="row">
                                    ...
                                    <div class="col-xl-6 mt-4 mb-4">
                                        <img id="showImage" class="rounded" style="width: 150px;" src="{{ $user->profile_image ? asset($user->profile_image) : asset('assets/img/images.png') }}" alt="profile">
                                        <input type="hidden" name="oldImage" value="{{ $user->profile_image }}" >
                                        <br>
                                        @if($user->profile_image)
                                            <button type="button" href="{{ route('members.deleteImage', $user->id) }}" class="btn btn-danger btn-sm mt-2" id="delete">Sil</button>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="row">
                                    <div class="col-xl-6">
                                        ...
                                        <input type="hidden" name="oldCV" value="{{ $user->user_cv }}" >
                                        @error('user_cv')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-xl-6 mt-4 mb-4">
                                        @if($user->user_cv)
                                            <a type="button" href="{{ url($user->user_cv) }}" class="btn btn-info" download="">CV'yi indir</a>
                                            <button type="button" href="{{ route('members.deleteCV', $user->id) }}" class="btn btn-danger" id="delete">CV'yi sil</button>
                                        @else
                            ...
                            <div class="col-md-4">
                                <a href="{{ route('members.index') }}" class="btn btn-secondary">Geri dön</a>
